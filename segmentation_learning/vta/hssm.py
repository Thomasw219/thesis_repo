...
class HierarchicalStateSpaceModel(nn.Module):
    def __init__(self,
        ...
        self.abs_post_bwd = RecurrentLayer(input_size=self.feat_size,
                                           hidden_size=self.abs_belief_size)
        self.obs_post_fwd = RecurrentLayer(input_size=self.enc_obs.embedding_size,
                                           hidden_size=self.obs_belief_size)
    ...
    def forward(self, obs_data_list, action_data_list, seq_size, init_size):
        #############
        # data size #
        #############
        ...
        enc_action_list = self.action_encoder(action_data_list)

        enc_combine_obs_action_list = self.combine_action_obs(
            torch.cat([enc_obs_list, enc_action_list], dim=-1)
        )

        ######################
        # boundary sampling ##
        ######################
        post_boundary_log_alpha_list = self.post_boundary(enc_combine_obs_action_list)
        ...
        obs_rec_mean = obs_rec_list[..., :action_data_list.shape[-1]]
        obs_rec_std = obs_rec_list[..., action_data_list.shape[-1]:]
